...
/*
TODO:
Kind1984 reports are not replaceable events, so need to process differently than 

- sql1: select kind1984EventId_new from events where kind=1984 and flaggedForProcessing=1
- for each row:
  ...
  - define pubkey, kind1984EventId_new, created_at_new
  - sql2: select kind1984EventId_old from users where pubkey=pubkey;
  - get event_old and event_new from s3 using keys: eventsByEventId/<kind1984EventId_old> and eventsByEventId/<kind1984EventId_new>
  - extract created_at_old and created_at_new from their respective events
  - if created_at_new > created_at_old, then:
    - sql3: update users set kind1984eventId, flaggedForKind1984EventProcessing=1 where pubkey;
  cleaning up:
  - sql4: update events set flaggedForProcessing=0 where eventId=kind1984EventId_new

usage:

http://localhost:3000/api/dataManagement/events/processKind1984Events?n=1

https://www.graperank.tech/api/dataManagement/events/processKind1984Events?n=1
...
export default async function handler(
  ...
  try {
    ...
    const sql1 = ` SELECT * FROM events where kind=1984 and flaggedForProcessing=1 `
    ...
    for (let x=0; x < Math.min(numEventsToProcess, aEvents.length); x++) {
      ...
      const kind1984EventId_new = oNextEvent.eventId
      ...
      if (aUsers.length == 1) {
        ...
        const kind1984EventId_old = oUserData.kind1984EventId
        if (kind1984EventId_old) {
          const params_get = {
            Bucket: 'grapevine-nostr-cache-bucket',
            Key: 'eventsByEventId/' + kind1984EventId_old,
          }
      ...
      if (created_at_new > created_at_old) {
        ...
        const sql3= ` UPDATE users SET kind1984eventId='${kind1984EventId_new}', flaggedForKind1984EventProcessing=1 WHERE pubkey='${pubkey}' `
        const results_sql3 = await connection.query(sql3);
        console.log(results_sql3)
      }

      // cleaning up 
      const sql4= ` UPDATE events SET flaggedForProcessing=0 WHERE eventId='${kind1984EventId_new}' `
    ...
    const response:ResponseData = {
      success: true,
      message: `api/dataManagement/events/processKind1984Events data:`,
      data: { 
    ...
    const response:ResponseData = {
      success: false,
      message: `api/dataManagement/events/processKind1984Events error: ${error}!`,
    }
