...
export default async function handler(
  ...
  if (!searchParams.observer) {
    const response:ResponseData = {
      success: false,
      message: `api/outwardFacing/getRatorsWithDos: no observer was provided`
    }
  ...
  if (!searchParams.observee) {
    const response:ResponseData = {
      success: false,
      message: `api/outwardFacing/getRatorsWithDos: no observee was provided`
    }
    res.status(500).json(response)
  }
  ...
  if (typeof observer == 'string' && verifyPubkeyValidity(observer) && typeof observee == 'string' && verifyPubkeyValidity(observee)) {
    ...
    try {
      ...
      const response:ResponseData = {
        success: true,
        exists: true,
        message: `api/outwardFacing/getRatorsWithDos data:`,
        metaData: {
    ...
    const response:ResponseData = {
      success: false,
      message: `api/outwardFacing/getRatorsWithDos: the provided observer and / or observee pubkey is invalid`,
      data: {
        observer, observee
      }
    }
